...
<\body>
  ...
  <\notation>
    ...
    <math|f\<in\><math|<math-ss|C>><around*|(|A,B|)>>, and
    <math|A:<math-ss|C>> for <math|A\<in\><math-ss|C>>.
  </notation>
  ...
  just by copying the object. In this way, we can extend a category to a
  larger one. But, we shall view the extended category as the same as the
  original, since this extension does not add new information to the original
  category. This means we shall view isomorphic objects as the same object.
  Regarding to morphisms, consider isomorphic objects <math|X> and <math|Y>,
  and another object <math|Z> of category <math|<math-ss|C>><math|>. If
  <math|X> and <math|Y> are viewed as one, then there will be bijections
  between <math|<math-ss|C><around*|(|X,Z|)>> and
  ...
  <\definition>
    ...
    natural isomorphism. We also say \P<math|F<around*|(|X|)>\<cong\>G<around*|(|X|)>>
    <strong|natural in> <math|X>\Q.
  </definition>
  ...
  <\remark>
    [Naturality] The word \Pnature\Q indicates figure <reference|figure:
    natural isomorphism v1>. This is the natural way of extending the
    isomorphism of objects to the bijection of morphisms. Indeed, the
    bijection can be read out directly from figure <reference|figure: natural
    isomorphism v1> as
